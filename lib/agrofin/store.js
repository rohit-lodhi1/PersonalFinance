...
const emptyData = () => ({
  ...
  incomeCategories: ['Salary','Farm','Freelance','Rental','Dividends','Other'],
  expenseCategories: ['Household','Farm','Utilities','Lifestyle','Health','Transport','Education','Other'],
})
...
export function StoreProvider({ children }) {
  ...
  const updateLoanTx = useCallback((loanId, txId, patch) => {
    setData(s => ({
      ...s,
      loans: (s.loans || []).map(l => l.id === loanId ? { ...l, txs: (l.txs || []).map(t => t.id === txId ? { ...t, ...patch } : t) } : l)
    }))
  }, [])
  const addCropInvestment = useCallback((cropId, inv) => {
    setData(s => ({
      ...s,
      crops: (s.crops || []).map(c => c.id === cropId ? { ...c, inputs: [{ id: uuid(), ...inv }, ...(c.inputs || [])] } : c)
    }))
  }, [])
  const updateCropInvestment = useCallback((cropId, invId, patch) => {
    setData(s => ({
      ...s,
      crops: (s.crops || []).map(c => c.id === cropId ? { ...c, inputs: (c.inputs || []).map(x => x.id === invId ? { ...x, ...patch } : x) } : c)
    }))
  }, [])
  const removeCropInvestment = useCallback((cropId, invId) => {
    setData(s => ({
      ...s,
      crops: (s.crops || []).map(c => c.id === cropId ? { ...c, inputs: (c.inputs || []).filter(x => x.id !== invId) } : c)
    }))
  }, [])
  const setBudget = useCallback((patch) => {
    setData(s => ({ ...s, budget: { ...(s.budget || { enabled: false, period: 'monthly', amount: 0 }), ...patch } }))
  }, [])
  const setCategories = useCallback((type, list) => {
    // type: 'incomeCategories' | 'expenseCategories'
    const cleaned = [...new Set(list.map(x => String(x || '').trim()).filter(Boolean))]
    setData(s => ({ ...s, [type]: cleaned }))
  }, [])

  const value = useMemo(() => ({
    token, me, managingUser, managingUserId, activeUserId,
    state: data, setState: setData,
    add, update, remove, addLoanTx, removeLoanTx, updateLoanTx,
    addCropInvestment, updateCropInvestment, removeCropInvestment,
    setBudget, setCategories,
    login, logout, authError, loading,
    users, refreshUsers, createUser, patchUser, deleteUser, manageAs,
  }), [token, me, managingUser, managingUserId, activeUserId, data, add, update, remove, addLoanTx, removeLoanTx, updateLoanTx, addCropInvestment, updateCropInvestment, removeCropInvestment, setBudget, setCategories, login, logout, authError, loading, users, refreshUsers, createUser, patchUser, deleteUser, manageAs])
